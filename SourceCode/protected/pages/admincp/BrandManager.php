		...
		if($this->IsValid)  // when all validations succeed
		{
			foreach($this->BrandList->Items as $item) {
				if ($item->colCheckBox->chkItem->Checked) {                                   
					...
					if ($activeRecord)
					{	
						$this->Response->redirect($this->Service->ConstructUrl("admincp.BrandForm",array("id"=>$activeRecord->ID,"alias"=>$activeRecord->Alias,"refUrl"=>urlencode($this->populateSortUrl($this->SortBy,$this->SortType)))));
						return;
					}
					else
					{

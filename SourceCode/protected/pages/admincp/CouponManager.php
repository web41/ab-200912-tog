...
class CouponManager extends TPage
{
	private $_maxPage = 1;
	...
	private $_sortType = "";
	private $_searchText = "";
	private $_sortable = array("coupon_id","coupon_code","coupon_amount","coupon_type");
	...
	{
		return TPropertyValue::ensureArray($this->Application->Parameters["COUPON_TYPES"]);
	}
	...
	protected function populateData()
	{
		...
		$criteria->Condition = "coupon_id in (select distinct coupon_id from tbl_coupon where coupon_id > 0 ";
	...
	protected function list_ItemCreated($sender, $param)
	{
		if ($param->Item->ItemType == "Item" || $param->Item->ItemType == "AlternatingItem")
		{
			if ($param->Item->Data)
			{
				$param->Item->colDeleteButton->Button->Attributes->onclick = 'if(!confirm("'.$this->Application->getModule("message")->translate("DELETE_CONFIRM","coupon",$param->Item->Data->Code).'")) return false;';
			}
		}
	}
			...
			case "delete":
				...
				if ($activeRecord)
				{
					try
					{
						...
						//var_dump();
						$this->Notice->Type = AdminNoticeType::Information;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_SUCCESS","Coupon",$activeRecord->Code);
					}
					catch(TException $e)
					{
						$this->Notice->Type = AdminNoticeType::Error;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_FAILED","Coupon",$activeRecord->Code);
					}
				}
				else
				{
		...
		if($this->IsValid)  // when all validations succeed
		{
			foreach($this->ItemList->Items as $item) {
				if ($item->colCheckBox->chkItem->Checked) {                                   
					...
					if ($activeRecord)
					{	
						$this->Response->redirect($this->Service->ConstructUrl("admincp.CouponForm",array("id"=>$activeRecord->ID,"alias"=>$activeRecord->Code,"refUrl"=>urlencode($this->populateSortUrl($this->SortBy,$this->SortType,"",false)))));
						return;
					}
					else
					{

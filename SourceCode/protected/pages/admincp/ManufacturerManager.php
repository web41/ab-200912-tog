...
class ManufacturerManager extends TPage
{
	private $_maxPage = 1;
	...
	private $_sortType = "";
	private $_searchText = "";
	private $_sortable = array("mf_id","mf_name","mf_email","mf_url");
	...
	protected function populateData()
	{
		...
		$criteria->Condition = "mf_id in (select distinct mf_id from tbl_manufacturer where mf_id > 0 ";
		...
		if($this->IsValid)  // when all validations succeed
		{
			foreach($this->ItemList->Items as $item) {
				if ($item->colCheckBox->chkItem->Checked) {                                   
					...
					if ($activeRecord)
					{	
						$this->Response->redirect($this->Service->ConstructUrl("admincp.ManufacturerForm",array("id"=>$activeRecord->ID,"alias"=>$activeRecord->Alias,"refUrl"=>urlencode($this->Page->populateSortUrl($this->Page->SortBy,$this->Page->SortType)))));
						return;
					}
					else
					{

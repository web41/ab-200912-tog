...
class OrderStatusManager extends TPage
{
	private $_maxPage = 1;
	...
	private $_sortType = "";
	private $_searchText = "";
	private $_sortable = array("status_id","status_name","status_code");
	...
	protected function populateData()
	{
		...
		$criteria->Condition = "status_id in (select distinct status_id from tbl_order_status where status_id > 0 ";
		...
		{
			$this->Notice->Type = AdminNoticeType::Information;
			$this->Notice->Text = $this->Application->getModule("message")->translate("ITEM_FOUND",0,"order status");
		}
	}
	...
	protected function list_ItemCreated($sender, $param)
	{
		if ($param->Item->ItemType == "Item" || $param->Item->ItemType == "AlternatingItem")
		{
			if ($param->Item->Data)
			{
				$param->Item->colDeleteButton->Button->Attributes->onclick = 'if(!confirm("'.$this->Application->getModule("message")->translate("DELETE_CONFIRM","order status",$param->Item->Data->Name).'")) return false;';
			}
		}
	}
			...
			case "delete":
				...
				if ($activeRecord)
				{
					try
					{
						...
						//var_dump();
						$this->Notice->Type = AdminNoticeType::Information;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_SUCCESS","Order status",$activeRecord->Name);
						$this->populateData();
					}
					catch(TException $e)
					{
						$this->Notice->Type = AdminNoticeType::Error;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_FAILED","Order status",$activeRecord->Name);
					}
				}
				else
				{
	...
	protected function btnDelete_Clicked($sender, $param)
	{
		if($this->IsValid)  // when all validations succeed
		{
			...
			try
			{
				...
				$this->Notice->Type = AdminNoticeType::Information;
				$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_ALL_SUCCESS","Order status");
			}
			catch (TException $e)
			{
				$this->Notice->Type = AdminNoticeType::Error;
				$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_ALL_FAILED","Order status");
			}
			$this->populateData();
		}
	}
		...
		if($this->IsValid)  // when all validations succeed
		{
			foreach($this->ItemList->Items as $item) {
				if ($item->colCheckBox->chkItem->Checked) {                                   
					...
					if ($activeRecord)
					{	
						$this->Response->redirect($this->Service->ConstructUrl("admincp.OrderStatusForm",array("id"=>$activeRecord->ID,"alias"=>$activeRecord->Alias,"refUrl"=>urlencode($this->populateSortUrl($this->SortBy,$this->SortType,"",false)))));
						return;
					}
					else
					{

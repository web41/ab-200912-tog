...
class PaymentMethodManager extends TPage
{
	private $_maxPage = 1;
	...
	private $_sortType = "";
	private $_searchText = "";
	private $_sortable = array("method_id","method_name");
	...
	protected function populateData()
	{
		...
		$criteria->Condition = "method_id in (select distinct method_id from tbl_payment_method where method_id > 0 ";
		...
		{
			$this->Notice->Type = AdminNoticeType::Information;
			$this->Notice->Text = $this->Application->getModule("message")->translate("ITEM_FOUND",0,"payment method");
		}
	}
	...
	protected function list_ItemCreated($sender, $param)
	{
		if ($param->Item->ItemType == "Item" || $param->Item->ItemType == "AlternatingItem")
		{
			if ($param->Item->Data)
			{
				$param->Item->colDeleteButton->Button->Attributes->onclick = 'if(!confirm("'.$this->Application->getModule("message")->translate("DELETE_CONFIRM","payment method",$param->Item->Data->Name).'")) return false;';
			}
		}
	}
			...
			case "delete":
				...
				if ($activeRecord)
				{
					try
					{
						...
						//var_dump();
						$this->Notice->Type = AdminNoticeType::Information;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_SUCCESS","Payment method",$activeRecord->Name);
					}
					catch(TException $e)
					{
						$this->Notice->Type = AdminNoticeType::Error;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_FAILED","Payment method",$activeRecord->Name);
					}
				}
				else
				{
	...
	protected function btnDelete_Clicked($sender, $param)
	{
		if($this->IsValid)  // when all validations succeed
		{
			...
			try
			{
				...
				$this->Notice->Type = AdminNoticeType::Information;
				$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_ALL_SUCCESS","payment method");
			}
			catch (TException $e)
			{
				$this->Notice->Type = AdminNoticeType::Error;
				$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_ALL_FAILED","payment method");
			}
			$this->populateData();
		}
	}
		...
		if($this->IsValid)  // when all validations succeed
		{
			foreach($this->ItemList->Items as $item) {
				if ($item->colCheckBox->chkItem->Checked) {                                   
					...
					if ($activeRecord)
					{	
						$this->Response->redirect($this->Service->ConstructUrl("admincp.PaymentMethodForm",array("id"=>$activeRecord->ID,"alias"=>$activeRecord->Alias,"refUrl"=>urlencode($this->Page->populateSortUrl($this->Page->SortBy,$this->Page->SortType)))));
						return;
					}
					else
					{

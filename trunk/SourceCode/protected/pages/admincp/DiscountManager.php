...
class DiscountManager extends TPage
{
	private $_maxPage = 1;
	...
	private $_sortType = "";
	private $_searchText = "";
	private $_sortable = array("discount_id","discount_name","discount_amount","s_date","e_date");
	...
	protected function populateData()
	{
		...
		$criteria->Condition = "discount_id in (select distinct discount_id from tbl_product_discount where discount_id > 0 ";
		if (strlen($this->SearchText)>0)
		{
			...
			{
				$searchQuery .= ($index>0 ? " or " : "")." discount_id like '%".addslashes($searchArray[$index])."%' or discount_name like '%".addslashes($searchArray[$index])."%'";
			}
		...
		{
			$this->Notice->Type = AdminNoticeType::Information;
			$this->Notice->Text = $this->Application->getModule("message")->translate("ITEM_FOUND",0,"discount");
		}
	}
			...
			case "delete":
				...
				if ($activeRecord)
				{
					try
					{
						...
						//var_dump();
						$this->Notice->Type = AdminNoticeType::Information;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_SUCCESS","Discount",$activeRecord->Name);
					}
					catch(TException $e)
					{
						$this->Notice->Type = AdminNoticeType::Error;
						$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_FAILED","Discount",$activeRecord->Name);
					}
				}
				else
				{
	...
	protected function btnDelete_Clicked($sender, $param)
	{
		if($this->IsValid)  // when all validations succeed
		{
			...
			try
			{
				...
				$this->Notice->Type = AdminNoticeType::Information;
				$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_ALL_SUCCESS","discount");
			}
			catch (TException $e)
			{
				$this->Notice->Type = AdminNoticeType::Error;
				$this->Notice->Text = $this->Application->getModule("message")->translate("DELETE_ALL_FAILED","discount");
			}
			$this->populateData();
		}
	}
		...
		if($this->IsValid)  // when all validations succeed
		{
			foreach($this->ItemList->Items as $item) {
				if ($item->colCheckBox->chkItem->Checked) {                                   
					...
					if ($activeRecord)
					{	
						$this->Response->redirect($this->Service->ConstructUrl("admincp.DiscountForm",array("id"=>$activeRecord->ID,"alias"=>$activeRecord->Alias,"refUrl"=>urlencode($this->Page->populateSortUrl($this->Page->SortBy,$this->Page->SortType)))));
						return;
					}
					else
					{
